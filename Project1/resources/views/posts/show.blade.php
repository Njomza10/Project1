...
@section('content')
    ...
    <div class="container">
        <div class="header">
            <div class="title">
                <p class="font-weight-bold">{{$posts->title}}</p>
            </div>
            <hr>
            <img
                ...
                class="img-circle" alt="." width="25px"
                height="25px">
            <p class="d-inline" style="color:blue;">{{$posts->user->name}}</p> |
            <small class="d-inline">{{$posts->created_at->format('d M Y')}}</small>
            <p class="pl-2  d-inline">
                ...
                </svg> {{$comments->count()}} comments
            </p>
        </div>
        <div class="container">
            <div class="img">
                <img class="card-img-top pt-3" src="{{$posts->image_url}}" alt="card-image-cap"
                     style="width: 550px; height:350px;">
            </div>
            <div class="category">
                <p class="text-muted">{{$posts->category->name}}</p>
            </div>
            <div class="body">
                <p>{{$posts->body}}</p>
            </div>
            <hr>
            <div class="comment">
                <p class=font-weight-bold>Display Comment:</p>
                <form action="{{route('comment.store' , $posts->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card col-md-7">
                        <div class="col-md-10">
                            <div class="form-row-group">
                                <div class="col-md-12">
                                    <label for="description" class="col-form-label"> Write Comment:</label>
                ...
                <div class="card bg-faded col-md-7">
                    @foreach($comments as $comment)
                        <div class="display-comment">
                            ...
                            <p class="d-inline">|</p>
                            <small class="d-inline">{{$comment->created_at->format('d M,Y')}}</small>
                            <p class="">{{ $comment->description }}</p>
                            <div class="float-left">

                                {{$comment->likes_count}}
                                <a href="{{route('like.store',$posts->id)}}">
                                ...
                                <a href="{{route('like.destroy',$posts->id)}}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                         fill="currentColor"
